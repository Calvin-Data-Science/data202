...
Spectrum <- 
  ...
  mutate(
    ...
    new_pos7 = rollmean(new_pos_per_day, 7, fill = NA, align = "right"),
    frac_pos7 = rollmean(frac_pos_new, 7, fill = NA, align = "right")
)
spectrum_tests_plot <- Spectrum %>% 
  gf_point(new_pos_per_day ~ centered_date, size = 0.5, alpha = 0.4) %>%
  gf_smooth(color = "maroon") %>% 
...
```{r}
orig_names <- names(Spectrum)
names(Spectrum) <- c('date', 'cum_tests', 'cum_pos', 'delta_tests', 'delta_pos', 'perc_pos_new',
                      'tests_per_day', 'new_pos_per_day')
names(orig_names) = names(Spectrum)
orig_names <- as.list(orig_names)

Spectrum <-
  Spectrum %>%
  set_variable_labels(.labels = orig_names)

Spectrum <-
  Spectrum %>%
  mutate(
    perc_pos_new = readr::parse_number(perc_pos_new) / 100,
    date = lubridate::mdy(paste(date, '/2020')))

spectrum_plot1 <-
  Spectrum %>%
  gf_point(new_pos_per_day ~ date, size = 0.5, alpha = 0.4) %>%
  gf_smooth(color = "maroon") %>%
  gf_labs(
    title = "Spectrum Health Positive Tests",
    y = get_variable_labels(Spectrum)[['new_pos_per_day']],
    x = ""
  ) %>%
  gf_lims(y = c(0, NA))
...
spectrum_plot2 <-
  Spectrum %>%
  gf_point(perc_pos_new ~ date, size = 0.5, alpha = 0.4) %>%
  gf_smooth(color = "maroon") %>%
  gf_labs(
    title = "Spectrum Health Positivity Rate",
    y = get_variable_labels(Spectrum)[['perc_pos_new']],
    x = ""
  ) %>%
  gf_refine(scale_y_continuous(labels = scales::percent, limits = c(0, NA))) 
```
...
plot_grid(spectrum_plot1, spectrum_plot2, ncol = 2)
```
