...
FivePeople <- read.csv('data/five-people.csv') |> mutate(score_positive = score >= 0)
FivePeople |> pander::pander()
```

Our goal is to predict the score (how much the person would like a given product on a scale
from -5 to 5) from some predictors (features) such as age, sex, and whether they use a 
...
Construct a tree that tells whether the person likes the product (score >= 0) or doesn't (score < 0).

**Note**: This doesn't have to be the *best possible* tree; just try to come up with some tree.

### A Regression Task

Construct a tree that predicts the value of the score.
