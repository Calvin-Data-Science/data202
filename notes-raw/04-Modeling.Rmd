...
FivePeople <- read.csv('data/five-people.csv') |>
  mutate(region = if_else(sex == "M", 1, 2), stars = (score + 5) / 2, liked = score >= 0) |> select(-score, -sex)
FivePeople |> pander::pander()
```

Our goal is to predict the score (how much the person would like a given product on a scale
from -5 to 5) from some predictors (features) such as age, region, and whether they use a 
...
Construct a tree that tells whether the person likes the product (stars >= 3) or doesn't (stars < 3).
