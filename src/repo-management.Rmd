...
get_assignment_files <- function(path, deploy_solutions) {
...
deploy_assignment_files <- function(files, repos = student_repos$portfolio_repo_full) {
  # Add to the reference and demo repos
  instructor_repos <- glue("{ghorg}/portfolio-{c('demo', 'reference')}")
  repo_add_file(instructor_repos, files, preserve_path = TRUE, overwrite = FALSE)
...
deploy_assignment <- function(path, deploy_solutions = FALSE) {
  files <- get_assignment_files(path, deploy_solutions = deploy_solutions)
  deploy_assignment_files(files)
}

deploy_assignment("ex/ex06-sad-plot")
#deploy_assignment("hw/hw02-vis")
...
roster_with_repos <- roster %>% 
  ...
  select(fname, last_name, slugname, github, email_address) %>% 
...
roster_with_repos %>% 
  rowwise() %>% 
  group_walk(~ system2('git', c('-C', .$worktree_full, 'pull')))
...
roster %>% 
  select(1:3) %>% 
  write_csv("gradescope_roster.csv")
```
