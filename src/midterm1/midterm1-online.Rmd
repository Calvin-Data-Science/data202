...
```{r cruise-control-by-type-sol, include=FALSE}
car_prices %>%
  group_by(type) %>% 
...
```{r cruise-control-by-type-sol-alt, include=FALSE}
car_prices %>%
  group_by(type, cruise) %>% 
  summarize(n = n()) %>% 
  group_by(type) %>%
  mutate(frac = n / sum(n)) %>% 
  filter(cruise == 1) %>% 
...
```{r cruise-control-repro, exercise=TRUE}
...
<!-- TODO: replace this one. Too similar to prev, and mean vs median gives different results. -->

```{r doors-engine-size-sol, include=FALSE}
car_prices %>%
...
<!-- TODO: don't allow this to use the result variable! -->
...
```{r join-repro, exercise=TRUE}

```


```{r distinct-non-matches, include=FALSE}
car_prices_and_eco %>% 
  distinct(model, combined) %>% 
  summarise(n = sum(is.na(combined)), frac = mean(is.na(combined)))
```

**Reproduce the following plot** using the `car_prices_and_eco` dataset.
...
```{r price-by-eco-repro, exercise=TRUE}
