...
getTeammates <- function(name) {
  roster_withteams %>% filter(preferred_name == name) %>% select(team) %>% left_join(roster_withteams, by="team")
}
```
...
```{r lookup-names}
local_repo_owners <- tibble(assignment = c("hw01", "hw02", "hw03", "hw04", "lab01", "lab02", "lab03", "lab04", "lab05", "datavis")) %>% 
  rowwise() %>% 
...
local_repo_owners %>% 
  mutate(
    ...
    pr_url = glue("{repo_url}/pull/1/"),
    md = glue("* [{name}]({repo_url}) ([feedback]({pr_url}))")
  ) %>%
  group_by(assignment) %>% 
...
local_repo_owners %>% 
  mutate(
    repo_url = glue("https://github.com/{ghorg}/{repo_short}"),
    pr_url = glue("{repo_url}/pull/1/"),
    md = glue("[{repo_short}]({repo_url}) ([feedback]({pr_url}))")
  ) %>%
  group_by(name) %>% 
