...
createAssignmentTemplate("lab/lab09-overfitting")
```
...
{
  username <- roster %>% filter(str_detect(preferred_full_name, "Sonali")) %>% pull(github)
...
expand_grid(
  ...
  mutate(repo = glue("lab09-{cohort}{repo_idx}")) %>% select(github, repo) %>% 
  {
    org_create_assignment(
      org = ghorg,
      user = .$github,
      repo = .$repo,
      team = .$repo,
      source_repo = glue("{ghorg}/lab09-template")
    )
  }
```
