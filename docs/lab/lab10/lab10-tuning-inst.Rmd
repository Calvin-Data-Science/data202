...
```{r}
spec <- modeling_results %>% slice_tail(n=1) %>% pluck("spec", 1)
...
sweep_model_examples <- function(model, dataset, vars_to_sweep, examples = slice_sample(dataset, n = 10)) {
  X <- map_dfr(vars_to_sweep, function(var_to_sweep) {
...
sweep_model_examples(model, ames_train, c("Longitude", "Latitude", "Gr_Liv_Area")) %>% 
  ggplot(aes(x = .sweep_val, y = .pred, group = .idx)) +
  geom_line() +
