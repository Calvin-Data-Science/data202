...
The data files are broken up by year. Here we read in the data for 2011; once you get everything working, you can use the commented code below to get 2012 data so you'll have the same two years that you worked with in Homework 1 and 2. The template already includes these lines, so you can just Run All to get the data loaded.
...
# Commented out to reduce memory and disk space usage.
...
5. How many rows does `daily_rides` have? How many *should* it have? (Don't try to fix this right now.)

6. Make `daily_rides_by_type`, a data frame just like `daily_rides` but breaking down the count by casual vs registered rider, like the data in hw02 did. The first few rows of the result should look like:
...
8. Show the same summary as the plot above, but in tabular form: what is the median number of rides taken by each type of rider for each day of the week?
...
Nnow that you've gotten the basic table, let's tweak it to make the result look better by adding two steps to the end of your pipeline. (**Make sure you get the above working before trying this.**)
