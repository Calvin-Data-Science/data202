...
We'll again be using the Capital Bike Share dataset. In hw3 we wrangled this
...
step; just use `linear_reg() %>% fit(...)`. This upgraded `add_predictions`
function can now take multiple models too (e.g., `data %>% add_predictions(model1, model2, model3)`,
and makes a tidy data frame with the predictions of each model for each data point,
and a column called `model` that gives the name of the model.
...
Find your repo in the [usual spot](https://github.com/Calvin-DS202-FA20/)
...
read_csv("../../data/bikeshare/day.csv", col_types = cols(
  ...
  rename(
    year = yr,
    date = dteday
  ) %>% write_csv("data/day-hw09.csv")
file.copy("data/day-hw09.csv", "../../../src/hw/hw09-modeling/data/day-hw09.csv")
```
...
(For this plot, and only this plot, the result will look cleaner if you specify
a `color` for `geom_line` outside of the aesthetic mapping. But that's optional.)
...
We can show the tree using the same method we used in Lab 9.
