...
Pull your portfolio repo and find the template in the `hwZ` folder.

## Supervised Learning

Build a decision tree classifier for the `type` of each storm in `nasaweather::storms` based on its `wind` speed and `pressure`.
Report its accuracy.
...
Consider the 4,000 biggest cities in the world, given by:
...
(Chapter 15, problem 9c)

Which baseball players have hit 500 home runs (`H`) OR 3000 hits (`H`) but have not (yet?) been inducted into the Baseball Hall of Fame?

Use the `Lahman` package. Consider:
...
1. (*problem 1a*) How many speaking lines are there in Macbeth? Speaking lines are identified by a line that starts with two spaces, then a string of capital letters (possibly including spaces) indicating the character's name, followed by a period.

2. (*problem 4a*) Find the 10 most popular boys' names in 2017 that end in a vowel. Use the `babynames::babynames` table. (Hint: `str_detect`.)
