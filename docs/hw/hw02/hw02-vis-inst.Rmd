...
We will be continuing our work with the [Capital Bikeshare dataset](https://archive.ics.uci.edu/ml/datasets/bike+sharing+dataset). Our goal continues to be to understand the pattern of ridership better to evaluate the current system and suggest potential improvements.

Towards that end, we will construct some more visualizations, this time using the more fine-grained *hourly* ridership data.

## Learning goals

Getting proficient at designing and implementing data visualizations takes practice. The goal of this assignment is to give you practice specifically with:
...
The two additional id columns mean that we've now broken down the data by `hour` of the day and rider type (`rider_type`). Some riders have `registered` for a Capital Bikeshare membership to get better rates. Other riders just bought a single trip or short-term pass, so we call them `casual` riders.
...
So: each row is the count of how many *rides* were completed in a given hour by a given type of rider. For each row, we have the following observed variables:[^2]
...
-   'weather_type`: see the [source data](https://archive.ics.uci.edu/ml/datasets/bike+sharing+dataset) for a description
...
```{r rides-by-date-membership, echo=FALSE}
...
Once you're done, **pick two plots** to leave in this section and remove all the others. **Compare and contrast** the design choices of the two plots: for which purpose is each one better, and why?

## Exercise 5: Distribution of Rides

Plot the distribution of the number of rides each day, broken down by rider type.

Try several different plot types. Keep the one that you think is most clear. Write a one-sentence interpretation of this plot.

```{r eval=FALSE, include=FALSE}
ggplot(daily_rides, aes(x = rides, color = rider_type)) + geom_density()
ggplot(daily_rides, aes(x = rides, y = rider_type)) + geom_boxplot()
ggplot(daily_rides, aes(x = rides, y = rider_type)) + geom_violin()
ggplot(daily_rides, aes(x = rides, y = rider_type)) + ggridges::geom_density_ridges(alpha = .5)
```

## Exercise 6: Distribution of Rides by Weekday

Plot the *distribution* of rides each hour. Your plot might look something like this:
...
(You might also try faceting, or using different plot types, etc.)

Write a one-sentence description of what the plot tells you about the data.

## Exercise 7: Plot of your choice

Pick another variable or two from the list of variables above. Make a plot of their relationship. Write a brief description of what you notice.
