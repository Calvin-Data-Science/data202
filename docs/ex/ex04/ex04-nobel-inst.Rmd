...
nobel_living_science %>% 
  mutate(origin = case_when(
    born_country == "USA" & country == "USA" ~ "USA-USA",
    born_country != "USA" & country == "USA" ~ "Other-USA",
    born_country == "USA" & country != "USA" ~ "USA-Other",
    TRUE ~ "Other-Other"
  ), origin2 = glue::glue("{born_country_us}-{country_us}")
  ) %>% 
  group_by(category, origin2) %>% 
  summarize(n = n()) %>% 
  ggplot(aes(x = n, y = origin2)) + 
...
6. In a single pipeline, filter for living STEM laureates who won their prize in the US, but were born outside of the US, and then create a frequency table for their birth country, `born_country`, and arrange the resulting data frame in descending order of number of observations for each country.
