...
gapminder_without_oceania <- filter(gapminder, continent != "Oceania")
show_examples <- FALSE
```
...
3. Set the `color` of the line to `red`.
...
1. Explain to your partner why the plot has the shape it does. (If you're not sure, try changing `geom_line` to `geom_point`.)
...
Discuss the differences between the different plots you get. What is the faceted plot good for? What is the not-faceted plot good for? What changes would you want to make to these plots to make them more legible?
...
Make a bar plot of the number of countries by continent. (You can `filter(year == 2007)` to get a data frame with one row per country.)
