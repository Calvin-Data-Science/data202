...
8.  Put the last line back to `num_pets`. Fill in the blank in the text line below the chunk. **Knit**, **Commit**, and **Push**, and check that your changes appear on GitHub. You can use the commit message "Add a count of the number of dogs." (Aside: try replacing the XXX by `` `r knitr::inline_expr('num_dogs')` `` and knitting again.)

9.  Change `Dogs` to `Cats` but do not rerun the chunk. Instead, Knit again. **Compare the output within the Source editor with the output in the knitted document.**
