...
- Change the year to your birth year or another interesting year in history.

- Use commas in the `x` axis labels. (Use the same `labels` code as in the `area` scale.)

- Try combining the `aes` lines together into a single `aes` command. Then try the same for the `labs`.
