...
* Using Pew data? See [notes on *weights*](https://cs.calvin.edu/courses/data/202/fa20/spss-tips.html)
...
* Measure accuracy on unseen data *without peeking at test set*
...
#model1_spec <- all_models$spec[[1]]
#model2_spec <- all_models$spec[[2]]
...
```{r ames-cv-anim, echo=FALSE, cache=TRUE, fig.show="animate", animation.hook="gifski"}
ames_train %>%
  vfold_cv(v = 10) %>%
  pull(splits) %>% 
...
model3_samples %>% collect_metrics(summarize = FALSE) %>% 
  ggplot(aes(x = .estimate, y = "model3")) + geom_point() + geom_rug()
```
