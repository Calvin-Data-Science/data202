...
```{r show-model3, fig.show="hide", fig.width=10}
...
```{r ref.label="show-model3", echo=FALSE}
```
...
recipe <-
  ...
  step_poly(starts_with("Gr_Liv_Area"), degree = 10)
...
ames_train %>%
  ...
  iwalk(function(split, split_idx) {
    print(
      ggplot(data = NULL, aes(x = Latitude, y = Longitude)) +
      geom_point(data = analysis(split), color = "green") +
      geom_point(data = assessment(split), color = "red") +
