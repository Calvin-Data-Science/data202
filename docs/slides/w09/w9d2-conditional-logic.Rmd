...
```{r echo=FALSE}
ames_train_2$remodeled[1:5] == "yes"
```
...
```{r echo=FALSE}
as.numeric(ames_train_2$remodeled[1:10] == "yes")
```
...
Its *sum* is the number of 1's. Its *mean* is the sum divided by the total number,
i.e., the *proportion* of values where the condition is true.

```{r}
ames_train_2 %>% summarize(
  sum = sum(remodeled == "yes"),
  mean = mean(remodeled == "yes")
)
```
...
Why are is there no column for `remodeled_no`?
...
```
if remodeled:
  Sale_Price = 22643 + 109 * Gr_Liv_Arera - 18424 * (1)
  Sale_Price = (59087 - 18424) + 380368 * Gr_Liv_Arera
else:
  Sale_Price = 22643 + 380368 * Gr_Liv_Arera
```
...
ggplot(baked_ames_train, aes(x = Gr_Liv_Area, y = Sale_Price, color = remodeled_yes)) +
  geom_point() +
...
## Can fit using the same interface

```{r}
decision_tree_fit <- decision_tree(mode = "regression", tree_depth = 2) %>% #<<
  set_engine("rpart") %>% 
...
```{r}
example_data <- tibble(Gr_Liv_Area = seq(0, 3000, length.out = 1000))
example_data <- 
  decision_tree_fit %>%
...
ggplot(ames_train, aes(x = Gr_Liv_Area, y = Sale_Price)) +
  geom_point() +
  geom_line(data = example_data, color = "red")
```
...
* *Conditional logic* by inventing special terms
...
* *To make a prediction*: follow a sequence of conditional logic to output a number.
