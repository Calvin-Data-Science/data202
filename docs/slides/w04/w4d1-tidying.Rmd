...
```{r total-cases-by-date, out.width="40%", fig.align='center'}
confirmed_global_long %>%
  group_by(date) %>%  #<<
...
```{r cases-by-country-broken, fig.width=15, fig.height=6}
confirmed_global_long %>%
...
```{r cases-by-country, fig.width=15, fig.height=6}
confirmed_global_long %>%
  filter(country_or_region %in%
    c("US", "Canada", "China", "Japan", "Korea, South", "Italy", "Germany", "Spain", "United Kingdom")) %>%
...
```{r join-debug}
most_recent_cases <- confirmed_global_long %>% 
  group_by(country = country_or_region) %>% 
  slice_tail(n = 1)
full_join( # <<
  population %>% select(country, population),
  most_recent_cases %>% select(country, confirmed),
  by = "country"
) %>%
