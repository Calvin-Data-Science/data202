...
display("Test set")
display(test_subset[all_cols].rename(columns=col_renames).style.hide(axis='index'))
```
...
3. Write down what your tree predicts for both `E` and `F` for each item in the test set.
