...
We'll use the classic Titanic dataset, augmented with some irrelevant (noise) features. The goal is to predict whether a passenger survived or not.[^This exercise is based in part on [this example from the sklearn docs](https://scikit-learn.org/stable/auto_examples/compose/plot_column_transformer_mixed_types.html).]
...
titanic_all = fetch_openml(
    "titanic", version=1, as_frame=True, parser="pandas"
)
titanic = titanic_all.frame.copy()
...
Hold out a test set. We'll use a large test set so that our estimates of generalization error are more accurate.
...
Let's show an example of what this preprocessor will do to our data:
