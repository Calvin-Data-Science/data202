...
```{python}
# make them categorical so they show up in the legend
cluster_labels = pd.Series(kmeans.labels_).astype("category")
ames_train['cluster'] = cluster_labels
```
