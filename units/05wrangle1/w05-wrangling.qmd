...
Source: Department of Education [College Scorecard](https://collegescorecard.ed.gov/data/)
...
students_by_state = (
  ...
  .sort_values("total_undergrads", ascending=False)
)
students_by_state
```
...
- `.size` of groups, not `value_counts`
  - **Caution**: the `size` on a `groupby` has a different meaning from `size` on a `DataFrame`.
