...
The data files are broken up by year. Here we read in the data for 2011, which is the first of the two years included in the dataset for previous exercises. **Download the data file [here](https://TODO)** and upload it to a `data` folder in your exercise folder.
...
1. Download the data file [here](https://TODO)
...
Make new columns for the year, month, and day of the ride.

To extract the year, month, and day from the `start_time` column, you can use the [`dt` accessor](https://pandas.pydata.org/docs/user_guide/basics.html#basics-dt-accessors) (short for "datetime"). Here's what one of them looks like, you try making the others:
...
rides['day'] = rides['start_time'].dt.day
rides.head(3)
```
...
```{python}
#daily_rides = rides.groupby(["year", "month", "day"], as_index=False).size()
daily_rides = (
    rides
...
When I first tried adding a trendline like we did, I hit a Plotly / pandas bug about `date` (not `datetime`) columns. See above for code for how to use `pd.to_datetime` to convert the column to a `datetime` column.

:::


## Part 3: Breaking down my rider type

Repeat Part 2, but now create a `daily_rides_by_type` data frame where you break down the counts by rider type, like we did in [Exercise 2](../01intro/ex02-bikeshare.qmd).

```{python}
daily_rides_by_type = rides.groupby(["date", "rider_type"], as_index=False).size()
