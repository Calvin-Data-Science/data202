...
## Concrete Mixture Strength {.smaller}
...
print(list(concrete_all.columns))
```
...
evaluate(concrete_test['strength'], concrete_test['predicted_strength']).to_frame('test')
```
...
We're not seeing any clear overall patterns.
Which mixtures are we worst at predicting? (ask a domain expert about these)
