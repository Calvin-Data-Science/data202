...
*Note*: your plots might show different ticks than the examples below, since the ticks that plotly draws depends on the amount of vertical space that it has. Generally the defaults are fine; if you really want to tweak them, see the [plotly course notes](../../notes/plotly.qmd#ticks).
...
- [ ] Do you have two plots?
- [ ] Are they both showing *distributions* of a continuous variable?
- [ ] Are they different plot types?
- [ ] Are they labeled in a way that humans can easily understand?
...
See the [plotly course notes](../../notes/plotly.qmd) for more details.
:::
...
- [ ] The plot uses the `country_count_by_continent` data
- [ ] The bars are ordered by number of countries
...
#| echo: true
median_order = (
  gapminder_latest.groupby("continent", as_index=False)['lifeExp'].median()
  .sort_values("lifeExp")['continent']
)
px.box(
  gapminder_latest,
...
**Correction**: plotly actually *does* support `categoryorder="median ascending"` or `categoryorder="median descending"`.
...
- [ ] There are two plots, each showing the relationship between a continuous and a categorical variable.
- [ ] A sentence or two is written comparing or contrasting the two plots.
...
- [ ] There are two plots, each showing the distribution of a continuous variable by year.
- [ ] A sentence or two is written comparing or contrasting the *distribution*s of the variables.
...
```{python}
# ensure that each year is individually labeled on the y axis
(
    px.violin(gapminder, y = "year", x = "gdpPercap", orientation = "h")
    .update_traces(side = "positive", width=15)
    .update_yaxes(tickvals = gapminder['year'].unique())
)
```
...
- [ ] I've checked off all the checklists above.
...
- [ ] The HTML document opens on my own computer.
- [ ] All my code is visible in my HTML document.
